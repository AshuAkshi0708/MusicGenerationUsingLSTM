...
model.load_state_dict(torch.load("Vanilla_model_10_3.pt"))
...
while len(song_sequence) < 3000:
    ...
    distribution = torch.nn.functional.softmax(output/temperature,dim = 1)
    input_position = torch.multinomial(distribution,num_samples = 1).numpy()
